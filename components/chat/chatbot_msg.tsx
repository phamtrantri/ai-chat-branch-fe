  ...
  return (
    ...
    >
      <div className="min-h-8 relative flex w-full flex-col items-start text-start break-words whitespace-normal">
        <div
          className={`relative max-w-full p-2 ${isHighlighted ? "border-2 border-amber-500" : ""}`}
        >
          <ReactMarkdown
            components={{
              code(props) {
        ...
        {message.content ? (
          <div className="flex flex-row gap-1 text-xs text-gray-500 px-2">
            <button
            ...
            message.child_conversations.length > 0 ? (
              <Dropdown className="max-w-[300px]">
                <DropdownTrigger>
                  <button
                    className="cursor-pointer hover:opacity-70 transition-all duration-200 font-normal bg-gray-100 px-1 py-0.5 rounded-sm"
                    type="button"
                  >
                    Threads
                  </button>
